...
Full-stack TypeScript app that tracks GitHub PR review comments, analyzes them with Claude CLI, and can auto-fix issues.
...
- **Services**: `server/services/` — Core logic: `db.ts` (SQLite schema + queries), `github.ts` (wraps `gh` CLI), `analyzer.ts` (Claude CLI invocation), `fixer.ts` (git operations + code fixes), `poller.ts` (5-min background sync), `jobs.ts` (activity tracking)
